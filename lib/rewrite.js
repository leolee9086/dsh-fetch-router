...
export function createRewriteRegistry({ warn = () => {} } = {}) {
   ...
   * 对一次请求依次跑所有改写器。
   *
   * 一个改写器自己出错，不该挡住别的改写器，也不该毁掉这次请求 —— 记一条 warn 跳过它。
   * 这不是静默：日志里有。
   *
  ...
  async function apply(request) {
    ...
    for (const [name, rewrite] of rewrites) {
      let next
      try {
        next = await rewrite({ ...request, body: current })
      } catch (error) {
        warn(`dsh-fetch-router: 改写器 "${name}" 出错: ${error instanceof Error ? error.message : String(error)}`)
        continue
      }
      if (next === undefined || next === null) continue
      if (typeof next !== 'string') {
        warn(`dsh-fetch-router: 改写器 "${name}" 返回的不是字符串,这次不改`)
        continue
      }
      current = next
      changed = true
