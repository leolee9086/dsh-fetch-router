/**
 * 配置到路由：把 routes 配置编译成 @leolee9086/sac-path-router 的中间件。
 * 本模块是纯函数，供单测直接调用。
 */
...
/**
 * 跑一遍登记的改写器，把结果写回 `ctx.requestBody`。
 *
 * 只有字符串 body 才改 —— 流、FormData、URLSearchParams 这些形态不碰：
 * 读它们要么得整段缓冲、要么得改语义，不是这一层该替调用方做的决定。
 * `ctx.requestBody` 的初值是调用方传进来的那份 body（fetch 入口设的），
 * 所以这里能安全地判断类型，不必去消费 request 的流。
 *
 * @param {object} ctx 路由上下文。
 * @param {object} hooks 记录与改写器来源。
 */
async function applyRewrite(ctx, hooks) {
  if (typeof hooks.rewrite !== 'function') return
  const body = ctx.requestBody
  if (typeof body !== 'string') return
  const scope = ctx.scope
  const next = await hooks.rewrite({ url: ctx.url.href, method: ctx.method, body, scope })
  if (next === undefined) return
  ctx.requestBody = next
  hooks.record({
    outcome: 'rewritten',
    // 表格的「规则」列直接显示这个值。不填的话它兜底成 passthrough ——
    // 明明改了请求却显示成透传,是比不显示更坏的误导。
    route: 'requestRewrite',
    // 与端点记录共用同一个回调；回调从 request 读取 URL 和方法。
    // 缺少它会让成功的改写在出网前因记录动作抛 TypeError。
    request: ctx.request,
    scope,
    detail: `请求体被改写(${body.length} → ${next.length} 字符)`,
  })
}
...
export function buildRouter(config, hooks) {
  ...
  // 内容改写是**中间件层**的事：它看的是 body，跟这条请求发去哪个 host 无关。
  // 端点规则是**路由层**的事：按 host + path 匹配，决定注头 / 改目标 / 本地应答。
  //
  // 两层各占一层。此前把改写判断塞在 fetch 入口外面，等于让端点路由的匹配结果
  // 去决定「body 能不能被改」，于是未命中端点规则的 host 连 body 都改不了。
  // 中间件注册在路由规则**之前**，所以它对每个请求都先跑一遍。
  router.use(async (ctx, next) => {
    await applyRewrite(ctx, hooks)
    await next()
  })
...
/**
 * 请求的 host 是否落在已配置的集合上（精确匹配或 `*.` 通配）。
 *
 * **拦截器不再用它做早退。** 早退的含义是「未命中端点规则的请求不构造 Request」，
 * 而那正是让内容改写无法对所有请求生效的原因 —— 改写是中间件层的事，它看的是 body，
 * 跟这条请求发去哪个 host 无关。保留这个函数是因为它对诊断有用（面板与规则表要回答
 * 「这条请求本来有没有可能被某条端点规则接手」），而且它有测试。
 *
 * @param {string} url 请求地址。
 * @param {Set<string>} hosts 已配置的 host 集合。
 * @returns {boolean} 是否落在已配置的 host 上。
 */
