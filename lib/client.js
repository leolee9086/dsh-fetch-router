    ...
    return {
      ...
      apply(ctx) {
        ...
        const Opener = (props) => {
          ...
          // 该槽位容器是 flex 行，同槽位其他占用者（如重启插件）自带整行控件组，
          // 因此本入口做成紧凑动作按钮：不争宽度，只占自身图标尺寸。
          const wrapper = { display: 'flex', alignItems: 'center', flex: 'none' }
          const button = wide
            ? {
                display: 'inline-flex', alignItems: 'center', gap: '6px',
                height: '36px', padding: '0 10px 0 8px', margin: 0,
                border: 'none', borderRadius: '10px', overflow: 'hidden',
                background: hover ? T.hover : 'transparent',
                color: T.ink, fontFamily: 'inherit', fontSize: '13px', cursor: 'pointer',
              }
            : {
