...
window.__ModuleLoader__.load({
  // 注册键必须是包名(含 scope):客户端模块表按图形行的 id 查工厂,行 id 就是包名。
  id: '@leolee9086/dsh-fetch-router',
  factory: (require) => {
    ...
    const outcomeColor = (outcome) => outcome === 'error' ? T.danger
      : outcome === 'mock' ? T.warn
        // 改写要用警示色,不能跟普通命中一样是绿的:请求被改过是要留意的事。
        : outcome === 'rewritten' ? T.warn
    ...
    const Monitor = (props) => {
      ...
      return h('div', { 'data-fetch-router': 'monitor', style: { display: 'flex', flexDirection: 'column', gap: '8px', padding: '10px', color: T.ink, fontSize: '12px' } },
        h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: '6px', alignItems: 'center' } },
          ...
          badge(t('rewritten'), counters.rewritten || 0, counters.rewritten ? T.warn : undefined),
      ...
      apply(ctx) {
        ...
        ctx.effect(() => ctx.locale.register(NS, {
          zh: {
            ...
            matched: '命中', passthrough: '透传', mock: '本地应答', error: '错误', rewritten: '改写',
          ...
          en: {
            ...
            matched: 'matched', passthrough: 'passthrough', mock: 'mock', error: 'errors', rewritten: 'rewritten',
